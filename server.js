...
const NODE_URL = String(process.env.NODE_URL || "").trim().replace(/\/+$/, "");
const BOOTSTRAP_NODE = String(process.env.BOOTSTRAP_NODE || "").trim().replace(/\/+$/, "");
...
app.get("/vendor/secp256k1.js", (req, res) => {
  res.sendFile(
    path.join(__dirname, "node_modules", "@noble", "secp256k1", "index.js"),
    error => {
...
async function createTables() {
  ...
  console.log("Table réseau KOA : OK");
}
...
async function getNodes() {
  ...
  // Inclut aussi les pairs éventuellement enregistrés en mémoire.
...
async function registerNode(nodeUrl) {
  ...
  if (!normalized) throw new Error("URL du nœud invalide.");
...
function validateExternalChain(rawChain) {
  try {
    ...
    return { valid: false, chain: null, totalMined: 0 };
  }
}
...
async function saveBlock(block) {
  if (!USE_DATABASE) {
    // Le bloc est déjà dans koala.chain. Le nœud mémoire n'a rien à écrire.
    return true;
  }
  ...
  try {
    ...
    for (const block of migratedChain) {
      await client.query(
        `
        UPDATE koa_blocks
        SET previous_hash = $1, hash = $2, nonce = $3, transactions = $4::jsonb
        WHERE block_index = $5
        `,
        [
          block.previousHash,
...
async function syncFromBootstrap() {
  ...
  // Remplace le Genesis aléatoire créé localement.
  koala.chain = validation.chain;
  koala.totalMined = validation.totalMined;
  koala.pendingTransactions = [];
  synchronized = true;
...
function pendingContains(transaction) {
  ...
  return koala.pendingTransactions.some(tx => transactionId(tx) === id);
}
...
app.get("/api/blocks", (req, res) => res.json(koala.chain));

app.get("/api/blocks/:index", (req, res) => {
  const block = koala.chain.find(item => item.index === Number(req.params.index));
...
app.post("/api/network/block", async (req, res) => {
  try {
    ...
    const existing = koala.chain.find(block => block.index === incomingBlock.index);
    ...
    if (
      ...
      return res.status(409).json({
        success: false,
        syncRequired: true,
        error: "Chaîne locale différente. Consensus nécessaire."
      });
    }

    const validation = validateExternalChain([...koala.chain, incomingBlock]);
...
app.post("/api/nodes/broadcast", async (req, res) => {
  try {
    ...
    const targets = currentNodes.filter(node => !sameNode(node, newNode));
...
app.post("/api/consensus", async (req, res) => {
  try {
    ...
    let bestLength = koala.chain.length;
    let source = NODE_URL || "local";
    const checked = [];

    for (const node of nodes) {
      try {
        const data = await fetchJson(`${node}/api/chain`);
        ...
        if (validation.valid && data.chain.length > bestLength) {
          bestChain = data.chain;
          bestLength = data.chain.length;
          source = node;
        }
      } catch (error) {
        checked.push({
    ...
    if (bestLength > koala.chain.length) {
      await replaceChain(bestChain);

      return res.json({
        success: true,
        replaced: true,
        message: "Blockchain locale synchronisée.",
        source,
        blocks: koala.chain.length,
        totalMined: koala.totalMined,
        checked
    ...
    res.json({
      success: true,
      replaced: false,
      message: "Blockchain locale déjà à jour.",
      blocks: koala.chain.length,
      totalMined: koala.totalMined,
      checked
    });
  ...
  try {
    if (USE_DATABASE) {
      ...
      if (!koala.isChainValid()) {
        throw new Error("Blockchain PostgreSQL invalide après restauration.");
      }
      ...
      // Important : le nœud mémoire doit récupérer le Genesis historique.
      await syncFromBootstrap();

      if (!koala.isChainValid()) {
        throw new Error("Blockchain invalide après synchronisation.");
      }
    }
    ...
    app.listen(PORT, "0.0.0.0", () => {
      ...
      console.log(`BOOTSTRAP_NODE : ${BOOTSTRAP_NODE || "NON CONFIGURÉ"}`);
      console.log(`Stockage : ${USE_DATABASE ? "PostgreSQL" : "mémoire"}`);
    ...
    console.error("Impossible de démarrer Koala Blockchain :", error);
    process.exit(1);
  }
}
